...
[학업 목표와 계획 계획]: 학업 중 이루고 싶은 주요 목표는 무엇인지, 구체적인 연구 계획이나 학습 전략이 있나요?  
- 산업 데이터와 딥러닝을 융합하여 실용적인 AI를 개발 할 수 있는 데이터 사이언티스트가 되고 싶으며, 주어진 데이터와 task에 적합한 방법론을 적용할 수 있는 능력을 키우는 것을 최우선 목표로 하고 있다. 또한 올바른 연구 방법 계획 그리고 연구의 중요성을 어필 할 수 있는 능력을 키우고 싶다. 이를 위해 많은 논문과 다양한 연구들을 탐구할 필요가 있으며, 인접 도메인에 대한 지식을 익혀 응용 방안을 탐구해나가겠다. 나아가 국내외 데이터 경진대회에 참여하여 다양한 데이터의 처리와 특성 공학의 노하우를 습득하며 딥러닝 이외의 머신러닝에 적합한 방식이 있다면 이것을 활용할 수 있도록 역량을 습득하겠다.
...
- 정형 데이터, 나아가 산업 데이터로의 관심
*국립농업과학원에서 과제를 수행하며 정형 데이터 기반의 딥러닝에 관심이 생겼다. 이미지와 딥러닝을 적용하려는 연구가 진행중이었는데 몇가지 문제가 생겼다.
...
- 석사 과정에 
* NLP와 관련해서는 Transformer 기반 모델을 활용해 문장형 수학 문제풀이 모델 설계 및 치과 임상진단을 위한 AI 환자를 설계했다. 
* 또한 NLP와 음성을 활용하여 Speech to Speech Translation를 연구를 위해 STT 모델의 구현과 더불어 Diffusion Model에서 파생된 방식을 활용해 Speech generation, Enhancement를 구현하며 다양한 딥러닝 모델과 응용을 학습하였다.
